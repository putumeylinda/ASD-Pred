...
Original file is located at
    https://colab.research.google.com/drive/13wkdTuRM-ltqG9cE5fYk8OioubVPS357
"""
...
def main():
    ...
    # --- Halaman 3: Hasil Prediksi ---
        ...
        # --- PERBAIKAN GRAFIK BAR (Label Dipendekkan) ---
        st.subheader("Probabilitas Hasil Prediksi")

        # Label pendek untuk memastikan tidak miring
        SHORT_LABELS = ["Tidak", "Iya"]

        # Membuat DataFrame dengan probabilitas
        # Urutan harus sesuai dengan [index_no, index_yes] jika kita ingin 'Tidak' muncul duluan di sumbu X
        prob_data = [proba_no, proba_yes]

        # Buat DataFrame vertikal (kategori di sumbu X, probabilitas di sumbu Y)
        final_chart_data = pd.DataFrame({
            "Probabilitas": prob_data
        }, index=SHORT_LABELS)

        # Tampilkan sebagai Bar Chart
        st.bar_chart(final_chart_data)

        # Keterangan untuk menjelaskan label pendek
        st.caption("Keterangan: 'Iya' berarti terdeteksi tanda-tanda ASD, 'Tidak' berarti tidak terdeteksi.")
