...
Original file is located at
    https://colab.research.google.com/drive/1mM2ykisNLIzzvTcMCpMMj44HL2xlNoyl
"""
...
Original file is located at
    https://colab.research.google.com/drive/1H59po3Go4uhjsxTfVSX_1OlEbnT_7uR6
"""
...
import numpy as np
import joblib
# Hapus import plotly.express karena menyebabkan ModuleNotFoundError
...
def main():
    ...
    # --- Halaman 2: Kuesioner ---
        ...
        with st.form("form_langkah_2"):
            ...
            for key, value in questions.items():
                # --- PERUBAHAN: Menggunakan st.radio (horizontal) ---
            ...
            if submitted2:
                ...
                try:
                    ...
                    # Mendapatkan probabilitas untuk kelas
    ...
    # --- Halaman 3: Hasil Prediksi ---
        ...
        classes = results["classes"] # Contoh: ['No', 'Yes']
        ...
        # --- PERBAIKAN GRAFIK BAR (Diubah menjadi Horizontal menggunakan trik transpose) ---
        st.subheader("Probabilitas Hasil Prediksi")

        # Membuat DataFrame dengan urutan yang diinginkan untuk sumbu Y: 'Iya (ASD)' di atas 'Tidak (Non-ASD)'
        chart_data = pd.DataFrame({
            "Iya (ASD)": [proba_yes],
            "Tidak (Non-ASD)": [proba_no]
        }).T # Transpose untuk membuat kategori ('Iya', 'Tidak') menjadi sumbu Y (Horizontal)

        # Streamlit bar_chart membaca indeks (kolom pertama di sini) sebagai sumbu Y.
        # Jika Anda ingin 'Iya' di atas 'Tidak', kita perlu membalik urutan indeks sebelum transpose.
        # Karena kita sudah menggunakan .T, kita harus membalik kolom.
        # Cara termudah: buat DataFrame langsung dengan urutan kolom yang terbalik untuk tampilan.

        # Buat ulang DataFrame dengan Probabilitas di kolom 0 dan baris sesuai urutan tampilan yang diinginkan (Iya di atas)
        final_chart_data = pd.DataFrame({
            "Iya (ASD)": proba_yes,
            "Tidak (Non-ASD)": proba_no
        }, index=["Probabilitas"]).T

        # Balik urutan baris agar 'Iya (ASD)' muncul di bagian atas
        final_chart_data = final_chart_data.iloc[::-1]

        # Tampilkan sebagai Bar Chart
        st.bar_chart(final_chart_data)
        # --- END PERBAIKAN GRAFIK BAR ---
