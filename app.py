...
Original file is located at
    https://colab.research.google.com/drive/1WeV5tcoXcc20b_s6TymkpROSVcTdAD6H
"""
...
import numpy as np
import joblib
import altair as alt # <-- TAMBAHAN BARU
...
def main():
    ...
    # --- Halaman 3: Hasil Prediksi ---
        ...
        # Tampilkan label probabilitas sebagai "Iya" / "Tidak"
        display_classes = [display_map_yes_no.get(c, c) for c in classes]

        # Buat DataFrame untuk chart
        df_chart = pd.DataFrame({
            "Probabilitas": prediction_proba,
            "Hasil": display_classes
        })

        # --- PERUBAHAN: Menggunakan Altair untuk chart horizontal dengan label horizontal ---

        # Buat chart
        chart = alt.Chart(df_chart).mark_bar().encode(
            x=alt.X('Probabilitas:Q', title='Probabilitas', axis=alt.Axis(format='%')), # Sumbu X sebagai probabilitas (diformat persen)
            y=alt.Y('Hasil:N', title='Prediksi', sort=None), # Sumbu Y sebagai "Iya"/"Tidak" (label horizontal otomatis)
            color=alt.Color('Hasil:N', legend=None) # Beri warna berbeda
        ).properties(
