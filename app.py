...
def main():
    ...
    # --- Halaman 3: Hasil Prediksi ---
        ...
        # --- SOLUSI GRAFIK DENGAN PLOTLY (Horizontal, label tegak) ---
        st.subheader("Probabilitas Hasil Prediksi")

        # 1. Siapkan DataFrame untuk Plotly
        chart_data_plotly = pd.DataFrame({
            # Kategori pada sumbu Y, nilai probabilitas pada sumbu X
            'Kategori': ['Tidak (Non-ASD)', 'Iya (Tanda ASD)'],
            'Probabilitas': [proba_no, proba_yes]
        })

        # 2. Buat Grafik Batang Horizontal menggunakan Plotly Express
        fig = px.bar(
            chart_data_plotly,
            x='Probabilitas',   # Nilai yang divisualisasikan
            y='Kategori',       # Label yang ditampilkan (sekarang tegak)
