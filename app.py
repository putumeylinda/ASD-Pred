...
Original file is located at
    https://colab.research.google.com/drive/1E8Gm6ry5nOA-fsi5Jx_Vi4_zpgBL94gB
"""
...
import numpy as np
import joblib
import plotly.express as px # Tambahkan impor untuk kontrol grafik yang lebih baik
...
def main():
    ...
    # --- Halaman 3: Hasil Prediksi ---
        ...
        # --- PERBAIKAN GRAFIK BAR (Diubah menjadi Horizontal menggunakan Plotly) ---
        # Membuat DataFrame dengan urutan yang diinginkan: 'Iya' di atas 'Tidak'
        chart_data = pd.DataFrame({
            "Hasil": ["Iya (ASD)", "Tidak (Non-ASD)"],
            "Probabilitas": [proba_yes, proba_no]
        })

        st.subheader("Probabilitas Hasil Prediksi")

        # Membalik urutan data agar 'Iya (ASD)' muncul di bagian atas grafik horizontal (lebih intuitif)
        chart_data_reversed = chart_data.iloc[::-1]

        # Menggunakan Plotly Express untuk membuat grafik batang horizontal
        fig = px.bar(
            chart_data_reversed, # DataFrame yang sudah dibalik
            x='Probabilitas',
            y='Hasil',
            orientation='h',
            text='Probabilitas',
            labels={'Probabilitas': 'Tingkat Probabilitas', 'Hasil': 'Hasil Prediksi'},
            color='Hasil',
            color_discrete_map={'Iya (ASD)': '#ef4444', 'Tidak (Non-ASD)': '#10b981'}, # Warna Tailwind: red-500 dan green-500
            height=250
        )

        # Format teks di dalam bar chart menjadi persentase dan letakkan di luar bar
        fig.update_traces(texttemplate='%{text:.2%}', textposition='outside')
        # Format sumbu X menjadi persentase
        fig.update_layout(xaxis_tickformat=".0%", uniformtext_minsize=8, uniformtext_mode='hide')
